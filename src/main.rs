    ...
    loop {
        ...
        let game_result = loop {
            ...
            for new_state in &new_states {
                values.push(minimax::minimax(*new_state, 2, &minimax::weighted_piececount));
            }
            let best = match game.turn() {
                game::Player::White => values.iter().max().unwrap(),
                game::Player::Black => values.iter().min().unwrap(),
            };
            let best_states: Vec<(&i64, game::GameState)> = values.iter().zip(new_states).filter(|(val, _)| *val==best).collect();
